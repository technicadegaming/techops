...
- **Import contract note (current):** fresh asset CSV intake treats manual/support URLs from uploads as non-authoritative hints (`manualHintUrl`, `manualSourceHintUrl`, `supportHintUrl`). Durable/manual-attached truth (`manualUrl`, `manualSourceUrl`, `supportUrl`, review/match flags) remains system-managed through enrichment + manual acquisition flows, with one temporary admin-only exception: CSV bootstrap attach mode may immediately persist direct `manualHintUrl` files and stamps provenance fields (`attachmentMode=csv_bootstrap`, `manualProvenance=csv_manual_hint_direct_attach`, `manualReviewState=manual_attached_bootstrap`).
