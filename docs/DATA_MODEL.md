...
- **Key fields (confirmed):** core identity fields such as `id`, `companyId`, `name`, `manufacturer`, `model`, `serialNumber`, `status`, `category`, `locationId`, `locationName`, `ownerWorkers`, `notes`; intake hint fields such as `alternateNames`, `subtitleOrVersion`, `playerCount`, `cabinetType`, `vendorOrDistributor`, `manualHintUrl`, `manualSourceHintUrl`, `supportHintUrl`, `manufacturerWebsite`, `externalAssetKey`; documentation/enrichment fields such as `manualLinks`, `manualLibraryRef`, `manualStoragePath`, `approvedManualIds`, `documentationSuggestions`, `supportResourcesSuggestion`, `supportContactsSuggestion`, `enrichmentStatus`, `enrichmentTerminalReason`, `enrichmentRequestedAt`, `enrichmentLastRunAt`, `reviewState`, `manualStatus`, `manualReviewState`, plus additive operator triage fields like `manualLibraryFlagged`, `manualLibraryFlaggedAt`, and `manualLibraryFlagReason`; maintenance scaffold fields under `maintenancePlan` (`intervalDays`, `checklist[]`, `jobPlanSummary`, optional `lastCompletedAt`, optional `nextDueAt`); service history fields such as `history` and `attachmentRefs`.
- **Import contract note (current):** fresh asset CSV intake treats manual/support URLs from uploads as non-authoritative hints (`manualHintUrl`, `manualSourceHintUrl`, `supportHintUrl`). Durable/manual-attached truth (`manualUrl`, `manualSourceUrl`, `supportUrl`, review/match flags) remains system-managed through enrichment + manual acquisition flows, with one temporary admin-only exception: CSV bootstrap attach mode may immediately persist direct `manualHintUrl` files into per-asset company-scoped bootstrap storage (`companies/{companyId}/asset-manual-bootstrap/{assetId}/...`) and stamps provenance fields (`attachmentMode=csv_direct_bootstrap`, `manualProvenance=csv_direct_manual_import`, `manualReviewState=manual_attached_bootstrap`).
- **Import/enrichment status note (current):** CSV intake sets `enrichmentStatus=queued` before kicking off enrichment work so imported rows only show active-search states when a real run is in-flight; import should not leave rows in `searching_docs` without active or enqueued work.
