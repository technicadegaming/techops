...
13. Admin imports now include a temporary “Bootstrap attach manuals from CSV hints” mode for testing: when enabled, direct `manualHintUrl` rows attempt immediate durable download/attach with explicit provenance (`attachmentMode=csv_bootstrap`, `manualProvenance=csv_manual_hint_direct_attach`, `manualReviewState=manual_attached_bootstrap`) while failures safely fall back to normal enrichment queueing.
