...
11. Post-selection manual handling now emits explicit state-machine telemetry and truthful failure reasons for fetch/validation/acquisition/persistence gaps (`selected_manual_fetch_failed`, `selected_manual_validation_failed`, `selected_manual_acquisition_failed`, `selected_manual_storage_write_failed`, `selected_manual_asset_persist_failed`, `selected_manual_selected_but_no_durable_fields_written`, `selected_manual_terminalized_inconsistently`) instead of collapsing everything into generic `no_manual`.
12. `manual_attached` review semantics are now guarded by durable attachment evidence (`manualLibraryRef` + `manualStoragePath` + `manualUrl`) so docs-found rows cannot be marked attached when manual fields are empty.
13. Admin imports now include a temporary “Directly attach CSV manuals (one-time bootstrap mode)” mode for testing: when enabled, direct `manualHintUrl` rows attempt immediate durable download/attach with explicit provenance (`attachmentMode=csv_direct_bootstrap`, `manualProvenance=csv_direct_manual_import`, `manualReviewState=manual_attached_bootstrap`) and import intentionally skips enrichment/research queueing in that mode.
...
- Added first-pass maintenance workflow scaffolding around assets (`maintenancePlan` interval/checklist/job-plan metadata) and surfaced due-state + cross-linked operations context so technicians can move between manuals, troubleshooting history, PM backlog, and related tasks from either asset or task views.
