...
export function applyAppearancePreference(pref = getDefaultAppearance()) {
  ...
  const surfaceText = pickReadableText(surface, readableText, getContrast(surface, '#0b1220') >= getContrast(surface, '#f8fafc') ? '#0b1220' : '#f8fafc');
  const primaryText = pickReadableText(primary, effectiveDark ? '#0b1220' : '#ffffff', effectiveDark ? '#f8fafc' : '#0b1220');
  const accentText = pickReadableText(accent, '#ffffff', '#0b1220');
  const danger = '#b91c1c';
  const success = '#166534';
  const warning = '#b45309';
  const dangerText = pickReadableText(danger, '#ffffff', '#0b1220');
  const successText = pickReadableText(success, '#ffffff', '#0b1220');
  const warningText = pickReadableText(warning, '#ffffff', '#0b1220');
  ...
  root.style.setProperty('--color-danger-text', dangerText);
  root.style.setProperty('--color-success-text', successText);
  root.style.setProperty('--color-warning-text', warningText);
