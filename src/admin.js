...
const AI_SETTINGS_SCHEMA = [
  { section: 'Enablement', fields: [{ key: 'aiEnabled', label: 'Enable Operations AI', help: 'Turns AI troubleshooting on for this company.' }, { key: 'aiAllowManualRerun', label: 'Allow manual rerun', help: 'Lead-or-higher can rerun AI from a task card.' }] },
...
export function renderAdmin(el, state, actions) {
  ...
  el.innerHTML = `
    ...
    <section class="item ${activeSection === 'imports' ? '' : 'hide'}" data-admin-section="imports"><h3>Workspace tools</h3><div class="row"><button id="downloadAssetsTemplate" type="button">Download assets CSV template</button><button id="downloadEmployeesTemplate" type="button">Download workers CSV template</button></div><div class="grid grid-2 mt"><label>Assets CSV<input id="assetCsvInput" type="file" accept=".csv" /></label><button id="applyAssetCsv" type="button">Import assets</button><label>Workers CSV<input id="employeeCsvInput" type="file" accept=".csv" /></label><button id="applyEmployeeCsv" type="button">Import workers</button></div>${adminUi.importSummary ? `<div class="tiny mt">${adminUi.importSummary}</div>` : '<div class="tiny mt">Choose a CSV to preview rows before import.</div>'}<pre id="importPreview" class="tiny">${adminUi.importPreview || ''}</pre></section>
