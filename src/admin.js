...
const canManageWorkerPin = ({ actorRole = '', targetRole = '', isSelf = false } = {}) => {
  const cleanActorRole = normalizeRole(actorRole);
  if (isSelf) return ['owner', 'admin', 'manager', 'lead'].includes(cleanActorRole);
  if (cleanActorRole === 'owner') return true;
  if (['staff', 'viewer'].includes(cleanActorRole)) return false;
  const actorRank = roleRank(cleanActorRole);
...
export function renderAdmin(el, state, actions) {
  ...
  const acceptedInvites = companyInvites.filter((invite) => invite.status === 'accepted');
  const invitesWithFailedAttempts = companyInvites.filter((invite) => Number(invite.failedAttempts || 0) > 0);
  ...
  const activePeopleRows = peopleRows.filter((row) => row.membershipId);
  const pendingInviteRows = pendingInvites.filter((invite) => `${invite.status || ''}`.trim().toLowerCase() === 'pending');
  ...
  const actorRole = normalizeRole(state.permissions?.companyRole || state.permissions?.role || 'staff');
  ...
  el.innerHTML = `
    ...
    const failedAttempts = Number(invite.failedAttempts || 0);
    const failureSummary = failedAttempts > 0 ? `${failedAttempts}${invite.lastFailedAttemptReason ? ` · ${invite.lastFailedAttemptReason}` : ''}` : '0';
    return `<tr><td>${invite.displayName || '—'}</td><td>${invite.email || '—'}</td><td>${formatRoleLabel(invite.role || 'staff')}</td><td>${invite.createWorkerProfile ? 'Yes' : 'No'}</td><td>${inviteCode ? `<code>${inviteCode}</code>` : '—'}</td><td>${invite.createdAt || '—'}</td><td>${invite.expiresAt || '—'}</td><td>${invite.status || 'pending'}</td><td>${failureSummary}</td><td><div class="action-row">${inviteCode ? `<button type="button" class="btn btn-ghost btn-small" data-copy-invite-code="${inviteCode}">Copy code</button>` : ''}${inviteCode ? `<button type="button" class="btn btn-ghost btn-small" data-copy-invite-link="${inviteCode}">Copy invite link</button>` : ''}${inviteCode ? `<button type="button" class="btn btn-ghost btn-small" data-copy-invite-message="${inviteCode}">Copy invite message</button>` : ''}<button type="button" data-revoke-invite="${invite.id}" class="btn btn-ghost btn-small">Revoke invite</button></div></td></tr>`;
  }).join('')}</tbody></table>` : '<div class="inline-state info mt">No pending invites.</div>'}</div><div class="list mt">${activePeopleRows.map((person) => {
    ...
    const targetRole = normalizeRole(person.role || person.worker?.role || 'staff');
    const canManagePersonPin = canManageWorkerPin({ actorRole, targetRole, isSelf: `${person.userId || ''}`.trim() === `${state.user?.uid || ''}`.trim() });
    return `<div class="item"><div class="row space"><b>${person.displayName || person.email || person.userId || person.id}</b><div class="state-chip-row">${roleChip}${renderStatusChip(accessStatus, statusTone)}${renderStatusChip(workerLabel, person.worker ? 'good' : 'muted')}</div></div><div class="tiny">${person.email || 'No email on file'} ${person.userId ? `| UID: ${person.userId}` : ''}</div><div class="tiny">Created: ${person.createdAt || '—'} · Accepted: ${person.acceptedAt || '—'} · Last login: ${person.lastLoginAt || '—'}</div>${invite ? `<div class="tiny mt">Invite status: ${(invite.status || 'pending').replace(/_/g, ' ')}${inviteCode ? ` · code <code>${inviteCode}</code> <button type="button" class="btn btn-ghost btn-small" data-copy-invite-code="${inviteCode}">Copy code</button>` : ''} <button type="button" class="btn btn-ghost btn-small" data-copy-invite-link="${inviteCode}">Copy invite text</button>${invite.status === 'pending' ? ` <button data-revoke-invite="${invite.id}" class="btn btn-ghost btn-small">Revoke</button>` : ''}${Number(invite.failedAttempts || 0) > 0 ? ` · failed attempts: ${Number(invite.failedAttempts || 0)}${invite.lastFailedAttemptAt ? ` (last ${invite.lastFailedAttemptAt})` : ''}` : ''}</div>` : ''}<div class="row mt">${person.membershipId ? `<form data-member-form="${person.membershipId}" class="row"><select name="role">${ACCESS_ROLE_OPTIONS.map((role) => `<option value="${role}" ${role === (person.role || 'staff') ? 'selected' : ''}>${formatRoleLabel(role)}</option>`).join('')}</select><select name="status"><option value="active" ${(person.status || 'active') === 'active' ? 'selected' : ''}>active</option><option value="inactive" ${(person.status || 'active') === 'inactive' ? 'selected' : ''}>inactive</option></select><button type="submit">Save access</button></form>` : ''}${person.membershipId ? `<button type="button" class="btn btn-ghost" data-disable-member-access="${person.membershipId}">Disable account</button><button type="button" class="btn btn-ghost" data-reactivate-member-access="${person.membershipId}">Reactivate access</button><button type="button" class="btn btn-danger" data-remove-member-access="${person.membershipId}">Remove app access</button>` : ''}${person.email ? `<button type="button" class="btn btn-ghost" data-send-password-reset="${person.email}" ${resetStatus === 'loading' ? 'disabled' : ''}>${resetLabel}</button>` : ''}${person.email || person.userId ? `<button type="button" class="btn btn-ghost" data-toggle-worker-profile="${person.id}" data-person-email="${person.email || ''}" data-person-user-id="${person.userId || ''}" data-person-name="${person.displayName || ''}" data-worker-enabled="${person.worker ? 'yes' : 'no'}">${person.worker ? 'Remove worker profile' : 'Make assignable worker'}</button>` : ''}</div>${person.worker ? `<form class="grid mt" data-worker-profile-form="${person.worker.id}"><label>Worker title (optional)<input name="title" value="${person.worker.title || ''}" /></label><label>Worker notes (optional)<input name="notes" value="${person.worker.notes || ''}" /></label><button type="submit">Save worker profile</button></form>` : ''}${(canManagePersonPin && person.worker && person.worker.assignable !== false) ? `<form class="grid mt" data-worker-pin-form="${person.worker.id}"><label>Location<select name="locationId" required>${(state.companyLocations || []).map((location) => `<option value="${location.id}" ${((state.companyLocations || []).length === 1 || location.id === (person.worker.defaultLocationId || '')) ? 'selected' : ''}>${location.name || location.id}</option>`).join('')}</select></label><label>PIN (4–8 digits)<input name="pin" inputmode="numeric" pattern="[0-9]{4,8}" minlength="4" maxlength="8" required autocomplete="off" /></label><button type="submit">${`${person.userId || ''}`.trim() === `${state.user?.uid || ''}`.trim() ? (person.worker.hasActivePin ? 'Reset my PIN' : 'Set my PIN') : (person.worker.hasActivePin ? 'Reset PIN' : 'Set PIN')}</button></form>` : ''}</div>`;
  }).join('') || '<div class="inline-state info">No active people yet.</div>'}</div></section>
    ...
    <section class="item ${activeSection === 'quiz' ? '' : 'hide'}" data-admin-section="quiz"><h3>Quiz & Training</h3><p class="tiny">Create, edit, and archive staff training quiz questions for Scoot Business accountability.</p><form class="grid grid-2 mt" data-quiz-question-form><label>Prompt<input name="prompt" placeholder="Question prompt" /></label><label>Category<input name="category" placeholder="operations, safety, policy" /></label><label>Choice A<input name="choiceA" /></label><label>Choice B<input name="choiceB" /></label><label>Correct choice<select name="correctChoice"><option value="A">A</option><option value="B">B</option></select></label><label>Points<input name="points" type="number" value="1" min="0" /></label><label><input name="active" type="checkbox" checked /> Active</label><button type="button" disabled>Save question (MVP scaffold)</button></form></section>
  ...
  el.querySelectorAll('[data-copy-invite-message]').forEach((button) => button.addEventListener('click', async () => {
    const code = `${button.dataset.copyInviteMessage || ''}`.trim();
    if (!code) return;
    const inviteMessage = `You're invited to Scoot Business. Create or sign in first, then use invite code ${code}. Join link: ${window.location.origin}/?invite=${encodeURIComponent(code)}`;
    await actions.copyInviteCode?.(inviteMessage);
  }));
  ...
  el.querySelectorAll('[data-worker-pin-form]').forEach((form) => form.addEventListener('submit', async (event) => {
    event.preventDefault();
    ...
    const result = await actions.setWorkerPin?.({ workerId: form.dataset.workerPinForm, locationId: payload.locationId, pin });
    if (result?.ok) {
      const pinInput = form.querySelector('input[name="pin"]');
      if (pinInput) pinInput.value = '';
    }
  }));
  el.querySelectorAll('[data-disable-member-access]').forEach((button) => button.addEventListener('click', async () => {
    await actions.disableMemberAccess?.(button.dataset.disableMemberAccess);
  }));
  el.querySelectorAll('[data-reactivate-member-access]').forEach((button) => button.addEventListener('click', async () => {
    await actions.reactivateMemberAccess?.(button.dataset.reactivateMemberAccess);
  }));
  el.querySelectorAll('[data-remove-member-access]').forEach((button) => button.addEventListener('click', async () => {
    await actions.removeMemberAccess?.(button.dataset.removeMemberAccess);
  }));
